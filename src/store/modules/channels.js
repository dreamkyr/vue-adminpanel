import { sendGet, sendDelete, sendPostJSON, sendPutJSON } from '../../config/api'
...
const actions = {
  ...
  fetchChannels ({commit}, payload) {
    ...
    return new Promise((resolve, reject) => {
      sendGet('/channels/search?letter=' + state.sort + '&expand=createdBy,modifiedBy&page=' + payload.page, null, null)
        .then((response) => {
  ...
  updateChannel ({commit, dispatch, getters}, payload) {
    ...
    return new Promise((resolve, reject) => {
      sendPutJSON('/channels/' + payload.id, payload)
        .then((response) => {
  ...
  deleteChannel ({commit, dispatch, getters}, payload) {
    return new Promise((resolve, reject) => {
      sendDelete('/channels/' + payload.id, null)
        .then((response) => {
          dispatch('setAlertData', {showAlert: true, content: response.english_name + ' is deleted!', alertClass: 'success'})
          resolve(response)
        })
        .catch((error) => {
          reject(error)
...
const mutations = {
  ...
  SET_CHANNEL_VALUE (state, payload) {
    ...
    if (payload.hasOwnProperty('icon')) {
      state.channel.icon = payload.icon
    }
  },
