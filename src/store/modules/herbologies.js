...
const actions = {
  ...
  searchHerbologies ({commit}, payload) {
    if (payload.type === 'prep') payload.type = 'flavours'
    if (state.sort === '') {
      payload.sort = ''
    } else {
    ...
    return new Promise((resolve, reject) => {
      sendGet('/' + payload.type + '/search?' + payload.sort + 'expand=createdBy,modifiedBy&page=' + payload.page, null, null)
        .then((response) => {
          commit('FETCH_HERBOLOGIES', response)
          let items = clone(state.herbologies)
          items['items'] = filter(orderBy(state.herbologies.items, ['created_at'], ['desc']), {'is_deleted': 0})
          resolve(items)
        })
        .catch((error) => {
          reject(error)
