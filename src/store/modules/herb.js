...
const actions = {
  ...
  searchHerbs ({commit}, payload) {
    ...
    return new Promise((resolve, reject) => {
      sendGet('/herbs/search?letter=' + payload.query + '&expand=categories,natures,flavours,channels,formulas,createdBy,modifiedBy&page=' + payload.page, null, null)
        .then((response) => {
          commit('FETCH_HERBS', response)
          resolve(response)
        })
  ...
  fetchHerb ({commit}, payload) {
    return new Promise((resolve, reject) => {
      sendGet('/herbs/' + payload.id + '?expand=categories,natures,flavours,channels,formulas,sources,species,cultivations,preparations,cautions,createdBy,modifiedBy', null, null)
        .then((response) => {
  ...
  updateHerbRelations ({commit, dispatch, getters}, payload) {
    return new Promise((resolve, reject) => {
      sendPostJSON('/herbs/import', payload)
        .then((response) => {
          dispatch('setAlertData', {showAlert: true, content: 'Herb relations updated', alertClass: 'success'})
          resolve(response)
        })
        .catch((error) => {
          reject(error)
  ...
  fetchHerbActivity ({commit}, payload) {
    ...
    return new Promise((resolve, reject) => {
      sendGet('/herbs/' + payload.id + '/activities?expand=user&page=' + page + '&per-page=' + count)
        .then((response) => {
          commit('FETCH_HERB_ACTIVITY', response.items)
...
const getters = {
  ...
  getHerbSpecieIds: (state) => {
    let herbSpecies = clone(state.herbSpecies)
    return reduce(herbSpecies, function (ids, specie) {
      ids.push(specie.id)
      return ids
    }, [])
  },
  getHerbActivity: (state) => {
    ...
    items['items'] = filter(orderBy(state.herbActivity.items, ['created_time'], ['desc']))
    return items
  },
...
const mutations = {
  ...
  FETCH_HERB (state, payload) {
    ...
    state.herbPreps = payload.preparations
    state.herbSpecies = payload.species
    state.herbCultivations = payload.cultivations
    state.herbCautions = payload.cautions
  },
