...
const actions = {
  ...
  fetchSearchUsers ({commit}, payload) {
    ...
    return new Promise((resolve, reject) => {
      sendGet('/users/search?sort=firstname&letter=' + state.sort + '&query=' + payload.query + '&page=' + payload.page, null)
        .then((response) => {
  ...
  fetchAllUsers ({commit}, payload) {
    return new Promise((resolve, reject) => {
      // sendGet('/users?expand=lastLogin,modifiedBy,roles&page=' + payload.page, null, null)
      sendGet('/users?sort=firstname&page=' + payload.page, null, null)
        .then((response) => {
          commit('FETCH_USERS', response)
          resolve(response)
        })
...
const getters = {
  ...
  getUsers: (state) => {
    ...
    items['items'] = filter(orderBy(state.users.items, [item => item.firstname ? item.firstname.toLowerCase() : '---'], ['asc']))
    return items
  },
