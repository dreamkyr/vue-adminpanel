...
const actions = {
  ...
  fetchGeneralCategory ({commit}, payload) {
    return new Promise((resolve, reject) => {
      sendGet('/' + payload.type, null, null)
        .then((response) => {
  ...
  searchGenerals ({commit}, payload) {
    if (state.sort === 'All') {
      commit('SET_ALPHABET_SORT', '')
    }
    return new Promise((resolve, reject) => {
      sendGet('/' + payload.type + '/search?letter=' + state.sort + '&expand=createdBy,modifiedBy,categories&page=' + payload.page, null, null)
        .then((response) => {
          if (payload.type !== 'channels') commit('FETCH_GENERALS', response)
          let items = clone(state.generals)
          items['items'] = filter(orderBy(state.generals.items, null, ['desc', 'desc']), {'is_deleted': 0})
...
const mutations = {
  ...
  SET_GENERAL_VALUE (state, payload) {
    ...
    if (payload.hasOwnProperty('categories')) {
      state.general.categories = payload.categories
    } else {
      state.general.categories = []
    }
