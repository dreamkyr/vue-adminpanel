<template>
  <div class="modal fade" id="modalHerbCultivations" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
          <h4 class="modal-title" id="myModalLabel">Herb Cultivations</h4>
        </div>
        ...
        <div class="modal-body no-padding" style="overflow: auto; position:relative; height:460px;">
          <table class="table table-hover">
            <tbody v-if="cultivations.items && cultivations.items.length > 0">
            <tr v-for="(item, key) in cultivations.items" :key="key">
              ...
              <td>
                <div class="btn-group pull-right">
                  ...
                  <button type="button" :class="selectedCultivationIds.indexOf(item.id) > -1 ? 'btn btn-success' : 'btn btn-default'" @click="toggleCultivation(item.id)"><i :class="selectedCultivationIds.indexOf(item.id) > -1 ? 'fa fa-check' : 'fa fa-plus'"></i></button>
                </div>
              </td>
            </tr>
            </tbody>
...
<script>
  ...
  export default {
    name: 'HerbCultivations',
    computed: {
      ...mapGetters({
        currentCultivationIds: 'getHerbCultivationIds',
    ...
    created () {
      this.fetchHerbCultivations()
    },
    watch: {
      currentCultivationIds: {
        handler () {
          this.selectedCultivationIds = Object.assign([], this.currentCultivationIds)
        }
      },
      flag: {
        handler () {
      ...
      modalType: {
        handler () {
          this.result = ''
          if (this.modalType === 0 && this.tabName === 'Cultivations') {
            this.flag = false
            this.result = 'success'
            this.$store.dispatch('setLoadingBar', 'block')
            this.$store.dispatch('setLoadingBarText', 'Loading...')
            this.fetchHerbCultivations()
          } else if (this.modalType === 5 && this.tabName === 'Cultivations') {
            if (this.flag === false) {
              this.$store.dispatch('setLoadingBar', 'block')
              this.$store.dispatch('setLoadingBarText', 'Loading...')
            }
    ...
    methods: {
      fetchHerbCultivations: function () {
        this.$store.dispatch('fetchCultivationsTotal', {query: 'query=' + this.query})
          .then((response) => {
            this.flag = true
            this.cultivations = response
          })
      },
      toggleCultivation (id) {
        let index = this.selectedCultivationIds.indexOf(id)
        if (index > -1) {
          this.selectedCultivationIds.splice(index, 1)
        } else {
          this.selectedCultivationIds.push(id)
        }
      },
      save: function () {
        this.selectedCultivations = []
        this.cultivations.items.map((item, ind) => {
          let index = this.selectedCultivationIds.indexOf(item.id)
          if (index > -1) {
            this.selectedCultivations.push(item)
          }
        })
