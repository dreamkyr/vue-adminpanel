    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title" id="myModalLabel" v-if="formula.id === ''">New Formula</h4>
          <h4 class="modal-title" id="myModalLabel" v-else>Edit Formula</h4>
        </div>
        ...
        <div class="modal-body" v-if = "this.modalType === 2">
          <div class="description"><b>Modified by</b> <a class="pull-right"><span class="badge bg-blue">{{formula.modified_by_name}}</span></a><p class="text-muted" data-toggle="tooltip" title="" data-original-title="Server Time">{{ formula.modified_at | toDate }}</p></div>
          <div class="description"><b>Created by</b> <a class="pull-right"><span class="badge bg-blue">{{formula.created_by_name}}</span></a><p class="text-muted" data-toggle="tooltip" title="" data-original-title="Server Time">{{ formula.created_at | toDate }}</p></div>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-default" data-dismiss="modal" ref="dialogClose" @click="close()">Cancel</button>
          <button type="button" class="btn btn-success" @click="save()" v-if = "this.modalType === 1">Create</button>
          <button type="button" class="btn btn-primary" @click="update()" v-if = "this.modalType === 2">Save</button>
        </div>
      </div>
    </div>
  </div>
    ...
    watch: {
      modalType: {
        handler () {
          ...
          this.formula.modified_by_name = this.selectedFormula.modified_by_name
          this.formula.modified_time = this.selectedFormula.modified_time
          this.formula.created_by_name = this.selectedFormula.created_by_name
          this.formula.created_time = this.selectedFormula.created_time
          this.formula.created_at = this.selectedFormula.created_at
          this.formula.modified_at = this.selectedFormula.modified_at
          this.result = ''
        }
      }
    },
