    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title" id="myModalLabel">Update formula</h4>
        </div>
        ...
        <div class="modal-body">
          <div class="description" v-if="formula.modifiedBy">
            <b>Modified by</b>
            <a class="pull-right"><span class="badge bg-blue">{{formula.modifiedBy.firstname}} {{formula.modifiedBy.lastname}}</span></a><p class="text-muted" data-toggle="tooltip" title="" data-original-title=""> {{ formula.modified_at | toDate }}</p>
          </div>
          <div class="description" v-if="formula.createdBy">
            <b>Created by</b>
            <a class="pull-right"><span class="badge bg-blue">{{formula.createdBy.firstname}} {{formula.createdBy.lastname}}</span></a><p class="text-muted" data-toggle="tooltip" title="" data-original-title=""> {{ formula.created_time | toDate }}</p>
          </div>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-default" data-dismiss="modal" ref="dialogClose">Cancel</button>
