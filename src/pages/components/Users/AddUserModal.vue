    ...
    <div slot="modalBody">
      <form role="form">
        ...
        <div class="form-group">
          <FBSelect
            @select2="createUser.type_id = $event"
            :selectId="'typeSelect'"
            :defaultClass="''"
            :options="types">
            <label for="typeSelect" slot="before">Type</label>
          </FBSelect>

          <div class="form-group">
            <label for="inputExperience" class="control-label">Roles</label>
            <div class="form-group">
              <div class="checkbox" v-for="item in roles" :key="item.name">
                <label :for="item.name">
                  <input type="checkbox" :id="item.name" :value="item.name" v-model="userRoles"/>
                  {{item.description}}
                </label>
              </div>
            </div>
